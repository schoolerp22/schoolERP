...
router.get("/:studentId/payments", async (req, res) => {
  try {
    ...
    // First check the new accounting_receipts collection
    const receipts = await db.collection("accounting_receipts")
      .find({ admissionNo: student.admission_no })
      .sort({ paidAt: -1 })
      .toArray();
    ...
    // Fallback: old payment_history array in student document
    return res.json(student.payment_history || []);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
});
